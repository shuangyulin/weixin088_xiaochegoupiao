<template>
  <div class="main-content">
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('xuesheng','排座') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="wodechengchexinxiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'排座':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','排座') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="wodechengchexinxiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'排座':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','排座') && contents.tableBtnIcon == 0" type="success" size="mini" @click="wodechengchexinxiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'排座':'' }}</el-button>
    ...
    <wodechengchexinxi-cross-add-or-update v-if="wodechengchexinxiCrossAddOrUpdateFlag" :parent="this" ref="wodechengchexinxiCrossaddOrUpdate"></wodechengchexinxi-cross-add-or-update>
...
import wodechengchexinxiCrossAddOrUpdate from "../wodechengchexinxi/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      wodechengchexinxiCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    wodechengchexinxiCrossAddOrUpdate,
  },
  methods: {
    contentStyleChange() {
    ...
    wodechengchexinxiCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.wodechengchexinxiCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','xuesheng');
      this.$nextTick(() => {
      this.$refs.wodechengchexinxiCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
